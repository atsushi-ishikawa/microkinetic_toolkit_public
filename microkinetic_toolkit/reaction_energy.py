...
parser.add_argument("--start", default=0,
                    help="elementary reaction number to start")
parser.add_argument("--end",   default=0,
                    help="elementary reaction number to end")
...
if calculator == "gaussian":
    ...
    calc_gas = Gaussian(label=dir, method=method, basis=basis,
                        scf="xqc", nprocs=nprocs, mem=mem)
    if SP:
        method_sp = "ccsd(t)"
        calc_gas_sp = Gaussian(label=dir, method=method_sp,
                               basis=basis, scf="xqc", nprocs=nprocs, mem=mem)
...
for irxn in range(rxnst, rxned):
    ...
    # working directories. set by label in Vasp calculator.
    dirs = {"reactant": None, "product": None}
        ...
        for imols, mols in enumerate(mol_set[irxn]):
            ...
            for imol, mol in enumerate(mols):
                ...
                if mol_type == 'adsorbed':
                    ...
                    #
                    if tmp == 'def':
                        defect = find_closest_atom(
                            surf_tmp, offset=offset*offset_fac)
                        ...
                        shift = (
                            xcom - tmp.positions[0, 0], ycom - tmp.positions[0, 1])
                        ...
                        add_adsorbate(surf_tmp, tmp, ads_height,
                                      position=ads_pos, offset=offset*offset_fac)
                        tmp = surf_tmp
            del surf_tmp
            #
            ...
            if calculator == "vasp":
                ...
                os.system(
                    'rm PCDAT XDATCAR EIGENVAL OSZICAR IBZKPT CHGCAR CHG WAVECAR REPORT >& /dev/null')

            # surf--nothing to do with vibration
            if mol_type != 'surf' and (ZPE[side] or IR[side]):
                # fix atoms for vibrations
                if mol_type == 'adsorbed':
                    c = FixAtoms(
                        indices=[atom.index for atom in surf if atom.tag == 1 or atom.tag == 2])
                ...
                if ZPE[side]:
                    ...
                    if not os.path.isdir(zpedir):
                        os.makedirs(zpedir)
                    # delta = 0.01 is default
                    vib = Vibrations(
                        tmp, delta=0.01, name=zpedir + "/" + "ZPE")
                    vib.run()
                ...
                if IR[side]:
                    ...
                    tmp.calc = calc_ir
                    tmp.calc.list_float_params[dipol] = tmp.get_center_of_mass(
                        scaled=True)

                    vib = Infrared(tmp, delta=0.01, name=irdir+"/"+"IR")
                    vib.run()
                    vib.write_spectra(
                        out=irdir + "_spectra.dat", start=1000, end=4000)
                    vib.write_mode()
                    vib.write_jmol()
    ...
    deltaE = np.sum(energies_bothside["product"]) - \
        np.sum(energies_bothside["reactant"])
    ...
    if TS:
        ...
        newatom1 = make_it_closer_by_exchange(
            atom1, atom2, thre=100.0)  # atom1 is exchanged
        # do not sort because POTCAR does not follow
        write('POSCAR_reac', newatom1)
        ...
        os.system('%s POSCAR_reac POSCAR_prod %d >& /dev/null' %
                  (nebmake, nimages))
    ...
    for side in ["reactant", "product"]:
        ...
        for imol, mol in enumerate(mols[irxn]):
            mol_join = '-'.join(mol)
            string = string + \
                "{0}_{1}".format(mol_join, '-'.join(sites[irxn][imol]))
